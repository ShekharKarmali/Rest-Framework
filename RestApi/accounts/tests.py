...
class AccountTestCase(APITestCase):
    ...
    def test_register_user_api_fail(self):
        ...
        data={
            'username':'cfedoe',
            'email'   :'cfedoe@gmail.com',
            'password':'learncode' 
        }
        response=self.client.post(url,data,format='json')
        self.assertEqual(response.status_code,status.HTTP_400_BAD_REQUEST) #this is used to correct it
    ...
    def test_register_user_api(self):
        ...
        # print(response.data.get("token",0))
    ...
    def test_login_user_api_fail(self):
        url=api_reverse('api-auth:login')
        data={
            'username' :'cfe.abc', #does not exists
            'password' :'yeahcfe', 
        }
        response=self.client.post(url,data,format='json')
